...
const { port, geminiApiKey } = require("./config");

const app = express();

/* ---------------- IMPORTANT: TRUST PROXY ---------------- */
/* Required when deploying on Render/Vercel/Railway */
...
/* ---------------- Retry Wrapper ---------------- */
async function callGeminiWithRetry(userMessage, retries = 3) {
  try {
    ...
    if (retries > 0 && err.status === 503) {
      ...
      return callGeminiWithRetry(userMessage, retries - 1);
    }
    throw err;
  }
}
...
app.post("/ask-ai", async (req, res) => {
  try {
    ...
    const result = await callGeminiWithRetry(userMessage);
    ...
    if (error.status === 503) {
      return res.status(503).json({
        error: "AI is busy right now. Please try again."
      });
    }
